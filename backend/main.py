...
async def debug_test_auth(request: Request):
    \"\"\"Temporary diagnostic endpoint to test authentication flow\"\"\"
    try:
        result = {}
        ...
        user_data = get_user_from_token(request)
        result['jwt_user'] = user_data
        
        # Test 2: Session data
        result['session_user_id'] = request.session.get('user_id')
        result['session_email'] = request.session.get('gmail_address')
        
        # Test 3: _request_user_email
        user_email = _request_user_email(request)
        result['computed_email'] = user_email
        
        # Test 4: is_logged_in check
        result['is_logged_in'] = is_logged_in(user_email) if user_email else False
        
        # Test 5: _require_user_id (the likely failure point)
        try:
            user_id = _require_user_id(request)
            result['user_id'] = user_id
        except Exception as e:
            result['user_id_error'] = f\"{type(e).__name__}: {str(e)}\"
        
        # Test 6: Database connection
        try:
            conn = _get_connection()
            cursor = conn.cursor()
            _execute(cursor, \"SELECT 1 as test\")
            test_row = cursor.fetchone()
            _release_connection(conn)
            result['db_connection'] = 'OK'
        except Exception as e:
            result['db_connection_error'] = f\"{type(e).__name__}: {str(e)}\"
